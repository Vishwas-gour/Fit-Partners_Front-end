...
import { sizes, categories, colors, materials, genders, stepLabels } from "../../Functions/RequeredArrays";
        ...
        try {
            ...
            if (step === 2) {
                ...
                if (!category || !gender || !material)
                    return alert("Fill all fields in Step 2");
            ...
            if (step === 3) {
                if (selectedSizes.length === 0)
                    return alert("Select at least 1 size");
                if (selectedColors.length === 0)
                    return alert("Select at least 1 color");
            ...
            if (step === 4) {
                ...
                // Reset form
                setProduct({
                    name: "", brand: "", stock: "", description: "", salePrice: "", costPrice: "", discountParentage: "", category: "", gender: "", material: "", weight: "",
                });
                setSelectedSizes([]);
                setSelectedColors([]);
                setImages([]);
        ...
        <form className="product-form" onSubmit={(e) => e.preventDefault()}>
            ...
            {step === 1 && (
                <>
                    ...
                    <input name="weight" value={product.weight} onChange={handleChange} placeholder="Weight (grams)" />
                    <input name="salePrice" type="number" value={product.salePrice} onChange={handleChange} placeholder="Sale Price" />
                    <input name="costPrice" type="number" value={product.costPrice} onChange={handleChange} placeholder="Cost Price" />
                    <input name="discountParentage" type="number" value={product.discountParentage} onChange={handleChange} placeholder="Discount %" />
                </>
            )}
            ...
            <div className="step-buttons">
                <button type="button" onClick={sendStepData}>{step === 4 ? (updateData ? "Update" : "Submit") : "Next"}</button>
